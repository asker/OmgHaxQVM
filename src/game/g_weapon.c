...
{
         //The following modification allows rants to spread hives while they have at least 1 evo.
	...
	if( ent->client->ps.persistant[ PERS_CREDIT ] == 0 )
	{
		...
		ent->client->ps.weaponTime += 500;
			...
			"print \"^3You have no evos to release the bees.\n\"" ) ); 
		return qtrue;
	}
	//trap_SendServerCommand( ent-g_entities, va(
		//"print \"^1Nade Dropped\n\"" ) );
	   //G_TeamCommand( PTE_HUMANS, "cp \"^1Alien Nuke Detected\" 1");
	   ent->s.weapon =  WP_HIVE;
	   //G_AddPredictableEvent( ent, EV_ALIEN_EVOLVE, 0 );
	
	   FireWeapon( ent );
                     trap_SendServerCommand( ent-g_entities, va(
			"print \"^2Bees have been released.\n\"" ) ); 
...
void FireWeapon( gentity_t *ent )
{
  ...
  switch( ent->s.weapon )
  {
    ...
    case WP_MASS_DRIVER:
      massDriverFire( ent );
      //Auto MD drop after fire- BOOSH(and FOXHOUND)
      BG_RemoveWeaponFromInventory(WP_MASS_DRIVER,ent->client->ps.stats);
      G_ForceWeaponChange(ent,WP_BLASTER); //switch to blaster
      ent->client->ps.persistant[ PERS_CREDIT ] = 0; //set creds to 0 
      break;
    case WP_LUCIFER_CANNON:
